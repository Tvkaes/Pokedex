...
interface UseStripeAuraSceneOptions {
  primary: Ref<string>
  secondary?: Ref<string | null | undefined>
}

const DEFAULT_PRIMARY = '#4338ca'

export function useStripeAuraScene({ primary, secondary }: UseStripeAuraSceneOptions) {
  ...
  const colorState = {
    ...
    accent: new THREE.Color(secondary?.value || primary.value || DEFAULT_PRIMARY),
  }
  ...
  function updateColors(primaryHex?: string, secondaryHex?: string | null) {
    colorState.primary.set(primaryHex || DEFAULT_PRIMARY)
    if (secondaryHex) {
    ...
    ribbonMaterials.forEach((material, index) => {
      ...
      if (ribbonColor instanceof THREE.Color) {
        ribbonColor.copy(colorState.accent).offsetHSL(index * 0.01 - 0.015, 0.015 * index, 0.008 * index)
      }
    })
  ...
  watch(
    () => [primary.value, secondary?.value ?? null] as const,
    ([nextPrimary, nextSecondary]) => {
      updateColors(nextPrimary, nextSecondary)
    }
  )
